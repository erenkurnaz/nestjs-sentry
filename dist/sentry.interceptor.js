...
const is_instantiable_type_1 = require("./utils/is-instantiable-type");
let SentryInterceptor = class SentryInterceptor {
    ...
    shouldReport(exception) {
        ...
        return filters.some(({ type, filter }) => {
            if ((0, utils_1.isStringArray)(type) && exception instanceof Error) {
                return !type.some((type) => exception.message.includes(type)) && (!filter || filter(exception));
            }
            return !((0, is_instantiable_type_1.isInstantiableType)(type) && exception instanceof type && (!filter || filter(exception)));
        });
    }
    ...
    captureHttpException(scope, http, exception) {
        const data = (0, node_1.addRequestDataToEvent)({}, http.getRequest(), { include: { ...this.options?.include } });
